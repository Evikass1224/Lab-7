...
namespace ProjectFileTasks
{
    ...
    class TaskSelection
    {
        ...
        private Dictionary<string, HashSet<int>> inputChocolatePreferences(int numberSweetTooth, HashSet<string> allChocolates)
        {
            ...
            List<string> chocolateList = allChocolates.ToList();  //список всех шоколадок
        ...
        private void dataEntryTask5()
        {
            ...
            Console.WriteLine("\nВведите имя файла (с расширением .dat) для сохранения данных об игрушках:");
            string fileName = InputFileName();
            ...
            //сериализация массива игрушек в файл
            serializer.Serialize(fileStream, toys);

            fileStream.Close();

            Console.WriteLine($"\nДанные об игрушках сохранены в {path}");
            TaskWithFilesSolutions.findToysForSpecificYears(path);
        }
